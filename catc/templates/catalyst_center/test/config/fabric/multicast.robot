...
{% if fabric_site.multicast is defined %}
{% if fabric_site.multicast.virtual_networks is defined %}
...
{% if fabric_site.multicast.replication_mode is defined %}
Verify Multicast Replication Mode for {{ fabric_site.name }}
    ${should_skip}=   Should Skip Site Validation   {{ fabric_site.name }}
    Pass Execution If   ${should_skip}   Replication mode for site {{ fabric_site.name }} not managed by this deployment state - skipping validation

    ${s}=   Get Cached Sites Data
    ${site_data}=   Get Value From Json   ${s.json()}   $.response[?(@.nameHierarchy=='{{ fabric_site.name }}')]
    ${site_exists}=   Evaluate   len(${site_data}) > 0
    Run Keyword If   not ${site_exists}   Fail   Site {{ fabric_site.name }} not found in Catalyst Center

    ${site_entry}=   Set Variable   ${site_data}[0]
    ${site_id}=   Get From Dictionary   ${site_entry}   id

    ${fabric_info}=   Get Value From Json   ${fabric_sites.json()}   $.response[?(@.siteId=='${site_id}')]
    ${fabric_exists}=   Evaluate   len(${fabric_info}) > 0
    Run Keyword If   not ${fabric_exists}   Fail   Fabric site not found for {{ fabric_site.name }}

    ${fabric_id}=   Get From Dictionary   ${fabric_info[0]}   id

    ${rm}=   GET On Session   CatalystCenter_Session   url=/dna/intent/api/v1/sda/multicast?fabricId=${fabric_id}
    Log   Multicast Replication Mode Response for {{ fabric_site.name }}: ${rm.json()}

    ${rm_data}=   Get Value From Json   ${rm.json()}   $.response[?(@.fabricId=='${fabric_id}')]
    Run Keyword If   not ${rm_data}   Fail   Multicast configuration for fabric {{ fabric_site.name }} not found
    Run Keyword And Continue On Failure   Should Be Equal Value Json String   ${rm_data}[0]   $.replicationMode   {{ fabric_site.multicast.replication_mode }}

{% endif %}

{% if fabric_site.multicast.wireless_multicast_enabled is defined %}
Verify Wireless Multicast for {{ fabric_site.name }}
    ${should_skip}=   Should Skip Site Validation   {{ fabric_site.name }}
    Pass Execution If   ${should_skip}   Wireless multicast for site {{ fabric_site.name }} not managed by this deployment state - skipping validation
    ...
    ${wm}=   GET On Session   CatalystCenter_Session   /dna/intent/api/v1/sda/fabrics/${fabric_id}/wirelessMulticast
    Log   Wireless Multicast Response for {{ fabric_site.name }}: ${wm.json()}

    ${multicast_enabled}=   Get Value From Json   ${wm.json()}   $.response.multicastEnabled
{% if fabric_site.multicast.wireless_multicast_enabled %}
    Run Keyword And Continue On Failure   Should Be True   ${multicast_enabled}[0]   msg=Wireless multicast should be enabled for {{ fabric_site.name }}
{% else %}
    Run Keyword And Continue On Failure   Should Not Be True   ${multicast_enabled}[0]   msg=Wireless multicast should be disabled for {{ fabric_site.name }}
{% endif %}
