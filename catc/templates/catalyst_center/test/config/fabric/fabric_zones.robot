...
Default Tags      config   catalyst_center   fabric   fabric_zones   site_specific
...
Verify Fabric Zone {{ fabric_zone.name }}
    ...
    ${should_skip}=   Should Skip Site Validation   {{ fabric_zone.name }}
    Pass Execution If   ${should_skip}   Fabric zone {{ fabric_zone.name }} not managed by this deployment state - skipping validation
    ...
    Should Be Equal Value Json String   ${zone_entry}   $.authenticationProfileName   {{ fabric_zone.authentication_template.name | default(defaults.catalyst_center.fabric.fabric_sites.authentication_template.name) }}
    {% endif %}
